...
A **Lomnia plugin** to extract and transform data from Firefox's browsing history
...
| Variable                     | Description                            |
| ---------------------------- | -------------------------------------- |
| `WEBSITE_SCHEMA_LOCAL`       | Path to the `Website` JSON schema.     |
| `WEBSITE_VISIT_SCHEMA_LOCAL` | Path to the `WebsitVisit` JSON schema. |

Examples:

```
WEBSITE_SCHEMA_LOCAL=/path/to/Website.schema.json
WEBSITE_VISIT_SCHEMA_LOCAL=/path/to/WebsiteVisit.schema.json
```
...
| -------------- | ----------------------------------------------------- |
| `--start_date` | Unix timestamp (seconds) to start fetching updates.   |
